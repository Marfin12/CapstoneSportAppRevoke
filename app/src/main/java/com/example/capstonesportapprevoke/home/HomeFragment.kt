...
import com.example.capstonesportapprevoke.utils.FragmentUtils
    ...
    override fun onCreateOptionsMenu(menu: Menu, inflater: MenuInflater) {
        ...
        searchView.setOnQueryTextListener(object : SearchView.OnQueryTextListener {
            override fun onQueryTextSubmit(query: String): Boolean {
                ...
                homeViewModel.teamAdapter.observe(viewLifecycleOwner
                ) { teamData ->
                    teamData.data?.let {
                        it.map { theTeam ->
    ...
    private fun observeCountryAdapter(
        ...
        with(binding) {
            homeViewModel.countryAdapter.observe(viewLifecycleOwner) { country ->
                if (country != null) {
                    when (country) {
                        is Resource.Loading -> incHomeHeader.headerProgressSportBar.visibility =
                            View.VISIBLE
                        is Resource.Success -> {
    ...
    private fun observeSportAdapter(sportAdapter: SportAdapter)
    {
        with(binding) {
            homeViewModel.sportAdapter.observe(viewLifecycleOwner) { sport ->
                if (sport != null) {
                    when (sport) {
                        is Resource.Loading -> incHomeEventSport.eventSportProgressBar.visibility =
                            View.VISIBLE
                        is Resource.Success -> {
